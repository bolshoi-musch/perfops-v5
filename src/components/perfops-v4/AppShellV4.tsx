...
import { products, projects } from "@/lib/perfops-v4-data";
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <footer className="border-t bg-card px-6 py-3 text-xs text-muted-foreground">
          <div className="mx-auto flex max-w-6xl items-center justify-between">
            <span>PerfOps · визуальный прототип v4</span>
                ...
                <li key={item.to}>
                  ...
                  {!collapsed && productsOpen && (
                    <ul className="mt-0.5 space-y-0.5 pl-7">
                      {products.map((p) => {
                        const productActive = path === `/v4/products/${p.id}`;
                        return (
                          <li key={p.id}>
                            <Link
