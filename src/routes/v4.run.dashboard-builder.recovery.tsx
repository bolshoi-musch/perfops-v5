...
export const Route = createFileRoute("/v4/run/dashboard-builder/recovery")({
  head: () => ({
    meta: [{ title: "Восстановление подготовки — PerfOps" }],
  }),
...
const copy: Record<RecoveryReason, { title: string; message: string }> = {
  missing: {
    title: "Сессия подготовки не найдена",
    message:
      "Мы не нашли сессию подготовки дашборда для этого проекта. Запустите подготовку заново.",
  },
  expired: {
    title: "Сессия подготовки устарела",
    message:
      "Сессия подготовки дашборда устарела и больше недоступна. Запустите подготовку заново.",
  },
};
...
function RecoveryPage() {
  ...
  return (
    ...
    <AppShellV4>
      <PageHeaderV4
        title={text.title}
        subtitle="Это служебная страница. Она не входит в основной поток подготовки дашборда."
      />
        ...
        <Card className="border bg-card shadow-none h-fit">
          <CardContent className="space-y-2 p-5">
            ...
            <p className="text-sm text-muted-foreground">
              Подготовка дашборда — это короткая сессия. Если её прервали или
              она устарела, продолжить с того же места не получится. Все ранее
              сохранённые источники и результаты остаются в Библиотеке.
            </p>
          </CardContent>
        </Card>
      </div>
