...
import { connectionTypes } from "@/lib/perfops-v4-data";
import { ArrowRight, Plug } from "lucide-react";

export const Route = createFileRoute("/v4/connections/")({
  head: () => ({ meta: [{ title: "Подключения — PerfOps V4" }] }),
          ...
          <Card key={c.id} className="border bg-card shadow-none">
            <CardContent className="flex h-full flex-col p-5">
              ...
              <Button variant="outline" size="sm" asChild className="self-start">
                <Link to="/v4/connections/$typeId" params={{ typeId: c.id }}>
